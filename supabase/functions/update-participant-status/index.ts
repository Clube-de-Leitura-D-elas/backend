...
Deno.serve(withSupabase<Database>({ auth: "user" }, async (req, ctx) => {
  ...
  let body: { participant_id?: unknown; is_active?: unknown } | null;
  try {
    body = await req.json();
  } catch {
  ...
  if (!isUuid(participantId)) {
    return Response.json({ error: 'missing/invalid "participant_id"' }, { status: 400 });
  }
  if (typeof isActive !== "boolean") {
    return Response.json({ error: 'missing/invalid "is_active"' }, { status: 400 });
  }
  ...
  return Response.json({ is_active: data.is_active });
}));
