 ...
 */ export async function requireFounder(ctx) {
  ...
  const { data: profile, error } = await ctx.supabaseAdmin.from("users").select("app_role").eq("user_id", authUserId).maybeSingle();
  if (error) {
