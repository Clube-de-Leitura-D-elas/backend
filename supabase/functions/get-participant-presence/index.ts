...
Deno.serve(withSupabase({
  ...
  const { data: memberships, error: membershipsError } = await ctx.supabaseAdmin.from("group_users").select("id").eq("user_id", participantId);
  if (membershipsError) {
  ...
  const { data: meetings, error } = await ctx.supabaseAdmin.from("meetings").select("id, date, meeting_group_users!inner(group_user_id, presence_status)").in("meeting_group_users.group_user_id", membershipIds).lte("date", new Date().toISOString()).order("date", {
    ascending: false
  }).limit(LAST_MEETINGS);
  if (error) {
  ...
  const items = (meetings ?? []).map((meeting)=>{
    const status = meeting.meeting_group_users[0]?.presence_status ?? "";
    return {
